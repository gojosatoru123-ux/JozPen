...
export const Navbarmenumobile = ({session}) => {
    const pathname = usePathname()
    return (
        <>
            <Link href="/" className={`flex gap-1 p-2 rounded-full ${pathname == '/' ? 'text-[#EE2B69] bg-gray-200' : ''}`}><Home /></Link>
            {session.isAuthorized && <Link href="/blog/create" className={`flex gap-1 p-2 rounded-full ${pathname == '/blog/create' ? 'text-[#EE2B69] bg-gray-200' : ''}`}><SquarePen /></Link>}
...
export const Navbarmenu = ({session}) => {
    console.log(session)
    const pathname = usePathname()
    return (
        <>
            <Link href="/" className={`flex gap-1 hover:text-[#EE2B69] transition-all ${pathname == '/' ? 'text-[#EE2B69] font-bold' : ''}`}><Home /> Home</Link>
            {session.isAuthorized && <Link href="/blog/create" className={`flex gap-1 hover:text-[#EE2B69] transition-all ${pathname == '/blog/create' ? 'text-[#EE2B69] font-bold' : ''}`}><SquarePen /> create</Link>}
